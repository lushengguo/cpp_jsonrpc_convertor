...
class Variable:
    ...
    def tuple_index(self, consider_function_return_type):
        ...
        if not consider_function_return_type:  # have to consider return type placeholder
            if self._function_return_type != "void":
                tuple_index += 1
        return tuple_index

    def deserialize(self, consider_function_return_type):
...
class Method:
    ...
    def server_side_return_type(self):
    ...
    def from_signature(signature):
        ...
        if args_str:
            ...
            for arg in args_list:
                ...
                if arg:
                    args.append(
                        Variable.from_signature_input_parameter(arg, ouput_rank_index, return_type))
                    if args[-1].is_output():
                        ouput_rank_index += 1
...
class RpcGenerator:
    def __init__(self, methods, interface_class_name, server_class_name, client_class_name, method_prefix='', method_suffix='_Rpc'):
    ...
    def generate_client_class_body(self):
        ...
        """
        return self.generate_interface_class_body() + body
    ...
    def generate_client_side_code(self):
        ...
        for method in self._methods:
            ...
            body += f"{method._return_type} {self._client_class_name}::{self.method_name(method)}({method.arg_list()}) {{\n"
            body += f"""
            try{{
                auto future = packio_client_->async_call("{method._rpc_internal_name}", 
                    std::make_tuple({', '.join(arg.client_side_serialize() for arg in method._args)}), net::use_future);
                {"auto rpc_result = " if not method.return_type_is_void() else ""} future.get();
                {deserialize_body}
            ...
            body += "}\n\n"
        return self.generate_client_class_body() + body

    def generate_server_class_body(self):
        callback_list = []
        for method in self._methods:
            types = ', '.join(
                f"{arg._type}" for arg in method._args)
            callback_list.append(
                f"std::function<{types}> {method.callback_name()};")
        body = f"""
    ...
    def generate_server_side_code(self):
        body = f"bool {self._server_class_name}::register_method(){{"
        for method in self._methods:
            args_str = ', '.join(
            ...
            if method._return_type != "void":
                tuple_part += f"result,"
            tuple_part += ",".join(
                f"{arg.server_side_serialize()}" for arg in method._args if arg.is_output())
            tuple_part += ");"

            body += f"""
                dispatcher()->add("{method._rpc_internal_name}",[]({args_str})->{method.server_side_return_type()}
                {{{service_body}{tuple_part}}});
            """

        body += "return true;}\n"

        return self.generate_server_class_body() + body
    ...
    def generate_test_code(self):
        ...
        for method in self._methods:
            ...
            for arg in method._args:
                ...
                callback_body += f"EXPECT_TRUE(memcmp({first_argument}, {second_argument}), 0);\n"
...
def entrance(interface_path, interface_class_name, client_class_name, server_class_name):
...
if __name__ == "__main__":
    entrance('hooks_rpc_interface.h', "hooks_rpc_interface", "executor_rpc_client", "plugin_rpc_server")
